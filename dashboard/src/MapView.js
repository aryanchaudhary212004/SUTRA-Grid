...
function ClusterLayer({ vehicles, onVehicleClick }) {
  const map = useMap();
  useEffect(() => {
    ...
    vehicles.forEach((v) => {
      ...
      marker.bindPopup(`<b>${v.vehicle_id}</b>`);
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      >
        ...
        <ClusterLayer vehicles={vehicles} onVehicleClick={setSelectedVehicle} />
        <HeatmapLayer vehicles={vehicles} />

        {corridor.map((signal, index) => {
          const lat = signal.lat || (28.676 + index * 0.001);
          const lng = signal.lng || (77.321 + index * 0.002);
          return (
            <Marker
              key={index}
              position={[lat, lng]}
              icon={signal.status === "GREEN" ? greenSignalIcon : redSignalIcon}
            >
              <Popup>
                🚦 Signal {signal.signalId} <br />
